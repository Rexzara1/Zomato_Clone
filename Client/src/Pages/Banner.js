...
class Banner extends React.Component{
    ...
    handleLocation = (e) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurants })  // set.State is used to update(edit/change) the data in this.state
        })
        ...
        return(
            suggestion.map((item, index) => (
                ...
                <li key={index} className="suggList" onClick={() => this.selectRestaurant(item._id)}>
                    <img src={ item.thumb } className="suggImg" />   {/*restaurant img*/}
    ...
    render(){
        const { locationData } = this.props;  //props is used to transfer data from one page to the other.
