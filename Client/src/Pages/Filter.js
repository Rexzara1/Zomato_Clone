...
class Filter extends React.Component{
    ...
    componentDidMount(){
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, mealtype: int  })  // set.State is used to update(edit/change) the data in this.state
        })
        ...
        axios({
            url: `http://localhost:5500/mealtypes/${int}`,
            method: 'GET',
            headers: { 'Content-Type': 'application/JSON'},
        })
        .then(res => {
            this.setState({ mealt: res.data.mealtype  })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
        // GET location API
        axios({
    ...
    handleLocation = (val) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, loca })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
    ...
    handleCuisine = (i) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, Cuisine: tempCuisine })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
    ...
    handleCost = (lcost, hcost) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, lcost, hcost })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
    ...
    handleSort = (sort) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, sort })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
    }
// Handle Page
    handlePage = (page) => {
        ...
        .then(res => {
            this.setState({ restaurant: res.data.restaurant, page })  // set.State is used to update(edit/change) the data in this.state
        })
        .catch((err => console.log(err)));
    }
    ...
    render(){
        ...
        return(
            <div>
                ...
                {/*<!--Filter Page-->*/}
                <div class="container mb-5">{console.log(mealt)}
                    <h2 class="filter-heading mt-3">{mealt.
name} Places in Mumbai</h2>
                    ...
                    <div class="filter-box mt-2 pb-4">
                        ...
                        <input type="radio" id="ltoh" name="Sort" value="Price low to high" onChange={() => this.handleSort(1)} /> <label for="ltoh" class="filter-content"  >Price low to high</label> <br />
                        <input type="radio" id="htol" name="Sort" value="Price high to low" onChange={() => this.handleSort(-1)} /> <label for="htol" class="filter-content"  >Price high to low</label> <br />
                        ...
                        restaurant.map((res) => {
                            return(
                                <div class="results" onClick={() => this.handleNavigate(res._id)}>
                                    <div class="d-flex">
                                        <div class="lt-box">
                                            <img src={res.thumb} alt="Pictures" class="img-fluid img-qs" />
                                        </div>
                                        <div class="rt-box">
